...
#         self.rnn=RNN(embedding_dim, hidden_dim, output_dim, n_layers, bidirectional, dropout)
#     def forward(self, x):
#         e = self.embedder(x)
#         o = self.rnn(e)
#         return o
...
#         self.encoder_p=encoder1
#         self.encoder_h=encoder2
#         self.final=nn.Linear(2*output_dim, output_dim)
#     def forward(self, p, h):
#         op, oh = self.encoder_p(p), self.encoder_h(h)
#         return self.final(torch.cat((op, oh), dim=1))

class SimpleSNLI(torch.nn.Module):
    def __init__(self, embedding_dim, hidden_dim, output_dim, n_layers, bidirectional, dropout):
        super(SimpleSNLI, self).__init__()
        self.rnn1=RNN(embedding_dim, hidden_dim, output_dim, n_layers, bidirectional, dropout)
        self.rnn2=RNN(embedding_dim, hidden_dim, output_dim, n_layers, bidirectional, dropout)
        self.final=nn.Linear(2*output_dim, output_dim)
    def forward(self, p, h):
        op, oh = self.rnn1(p), self.rnn2(h)
        return self.final(torch.cat((op, oh), dim=1))
...
def tokenize_sentence(s):
    ...
    indexed = [inputs.vocab.stoi[t] for t in tokenized]
    tensor = torch.LongTensor(indexed).to(device)
    tensor = tensor.unsqueeze(1)
    return tensor
pemb=tokenize_sentence("A black race car starts up in front of a crowd of people.")
hemb=tokenize_sentence("A man is driving down a lonely road.")
