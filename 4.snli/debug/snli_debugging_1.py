...
class SNLIClassifier(nn.Module):
    ...
    def forward(self, prem_embed, hypo_embed):
...
with torch.no_grad():
    model.eval()
    pred=torch.argmax(model(p_emb, h_emb))
pred
model.train()
p_dash=torch.zeros_like(p_emb)
h_dash=torch.zeros_like(h_emb)
sp=p_dash + k * (p_emb - p_dash) / m
k=1
m=300
sp=p_dash + k * (p_emb - p_dash) / m
sh=h_dash + k * (h_emb - h_dash) / m
so=model(sp,sh)
so.size()
so
p_emb.requires_grad
h_emb.requires_grad
sg=torch.autograd.grad(so[0,pred], (p_emb, h_emb), retain_graph=True)
sg=torch.autograd.grad(so[0,pred], p_emb, retain_graph=True)
sg=torch.autograd.grad(so[0,pred], sp, retain_graph=True)
p_emb, h_emb = embed_pair(s1_premise,s2_hypothesis,'')
model.train()
model.zero_grad()
so=model(p_emb,h_emb)
so
sg=torch.autograd.grad(so[0,2], (p_emb,h_emb), retain_graph=True)
sg=torch.autograd.grad(so[0,2], (p_emb,h_emb))
